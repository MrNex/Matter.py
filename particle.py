...
class Particle(Object):
	...
	solid_max_offset = 20
	solid_max_velocity = 19
	solid_max_ind_force = 7
	

	liquid_repel_force = 1
	liquid_max_ind_force = 1
	liquid_max_net_force = 2
	liquid_max_velocity = 20
	liquid_containment_force = 10
	liquid_containment_range = 2

	gas_repel_force = 2
	gas_containment_force = 1
	gas_containment_range = 2


	@classmethod
	def update_constants(cls):
		cls.solid_max_translation = cls.particle_radius / 10
		cls.solid_max_offset = cls.particle_radius * 2
		cls.solid_max_velocity = cls.solid_max_offset - 1
		cls.solid_max_ind_force = cls.particle_radius / 1.5
	

		cls.liquid_repel_force = cls.particle_radius / 10
		cls.liquid_max_ind_force = cls.particle_radius / 10
		cls.liquid_max_net_force = cls.particle_radius / 5
		cls.liquid_max_velocity = cls.particle_radius * (cls.particle_radius / 5)
		cls.liquid_containment_force = cls.particle_radius / 2.5 
		cls.liquid_containment_range = cls.particle_radius / 4

		cls.gas_repel_force = cls.particle_radius / 5
		cls.gas_containment_force = cls.particle_radius / 10
		cls.gas_containment_range = cls.particle_radius / 5
	...
	def solid_update(self, _system_position):
		...
		if(_dist > Particle.solid_max_offset) :
			#print('seeking lattice')
			if(self.identifier == 2 or self.identifier == 6):
				print('seeking: ' + str(_lattice_x_pos) + ', ' + str(_lattice_y_pos))
	...
	def liquid_update(self, _system_position, _system_radius):
		...
		_containment_force = self.limit_force(_containment_force, Particle.liquid_max_ind_force)
	...
	#def gas_update(self, _system_position):
